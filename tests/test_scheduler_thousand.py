...
def test_scheduler(test_db):
    # 10000个16核, 不限制共享数
    group, pod = _create_data(10, -1, 10000)


    start = time.time()
    assert get_max_container_count(group, pod, ncore=1, nshare=0) == 160000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=2, nshare=0) == 80000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=3, nshare=0) == 50000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=4, nshare=0) == 40000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=5, nshare=0) == 30000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=1, nshare=5) == 100000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=2, nshare=5) == 60000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=3, nshare=5) == 40000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=1, nshare=1) == 140000
    print time.time() - start

    start = time.time()
    assert get_max_container_count(group, pod, ncore=2, nshare=1) == 70000
    print time.time() - start

    start = time.time()
    assert len(average_schedule(group, pod, ncontainer=100, ncore=2)) == 100
    print time.time() - start

    start = time.time()
    assert len(average_schedule(group, pod, ncontainer=130, ncore=0, nshare=5)) == 130
    print time.time() - start
    ...
    start = time.time()
    r = average_schedule(group, pod, ncontainer=10000, ncore=2)
    print time.time() - start
    assert len(r) == 10000
    ...
    start = time.time()
    r = average_schedule(group, pod, ncontainer=10000, ncore=1, nshare=5)
    print time.time() - start
    assert len(r) == 10000
    assert sum(i[1] for i in r.keys()) == 10000
    for (host, count), cores in r.iteritems():
    ...
    start = time.time()
    r = average_schedule(group, pod, ncontainer=10000, ncore=2, nshare=3)
    print time.time() - start
    assert len(r) == 10000
    assert sum(i[1] for i in r.keys()) == 10000
    for (host, count), cores in r.iteritems():
    ...
    start = time.time()
    assert len(centralized_schedule(group, pod, ncontainer=100, ncore=2)) == 13
    print time.time() - start
    start = time.time()
    assert len(centralized_schedule(group, pod, ncontainer=130, ncore=0, nshare=5)) == 5
    print time.time() - start

    start = time.time()
    r = centralized_schedule(group, pod, ncontainer=10000, ncore=1)
    print time.time() - start
    assert len(r) == 625
    assert sum(i[1] for i in r.keys()) == 10000
    for (host, count), cores in r.iteritems():
    ...
    start = time.time()
    r = centralized_schedule(group, pod, ncontainer=100, ncore=2)
    print time.time() - start
    assert len(r) == 13
