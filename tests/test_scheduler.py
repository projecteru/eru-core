...
def test_centralized_schedule(test_db):
    ...
    assert len(centralized_schedule(pod, ncontainer=100, ncore=2)) == 0
    assert len(centralized_schedule(pod, ncontainer=130, ncore=0, nshare=5)) == 0

    r = centralized_schedule(pod, ncontainer=10, ncore=1)
    assert len(r) == 1
    assert sum(i[1] for i in r.keys()) == 10
    for (host, count), cores in r.iteritems():
        assert count == 10
    ...
    r = centralized_schedule(pod, ncontainer=9, ncore=2)
    assert len(r) == 2
    assert sum(i[1] for i in r.keys()) == 9
    for (host, count), cores in r.iteritems():
        assert count in (1, 8)
